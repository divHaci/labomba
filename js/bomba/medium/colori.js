...
function creaColori() {
  ...
  display.style.textShadow =
    "0px 0px 1vh " + colori_colonna_hex[soluzione_colonna];
  display.innerHTML =
    colori_nome[Math.floor(Math.random() * colori_nome.length)];
  ...
  while (count < 5) {
    ...
    var random_color =
      colori_bottoni_hex[Math.floor(Math.random() * colori_bottoni_hex.length)];
    ...
    if (usciti.indexOf(random_color) == -1) {
      ...
      if (soluzioni[soluzione_colonna].indexOf(random_color) == -1) {
        ...
        buttons_container.children[count].setAttribute(
          "onclick",
          "checkColore('wrong')"
        );
        count = count + 1;
  ...
  buttons_container.children[correct_button_n].style.backgroundColor =
    soluzioni[soluzione_colonna][random_solution];
  buttons_container.children[correct_button_n].setAttribute(
    "onclick",
    "checkColore('right')"
  );
...
function checkColore(button) {
  if (button == "wrong") {
    ...
    ledContainer.children[livello].style.webkitBoxShadow =
    ...
    ledContainer.children[livello].style.boxShadow =
      "0px 0px 2vw 0.5vw #fde910";
    livello = livello + 1;
    right.pause();
    right.currentTime = 0;
    ...
    if (livello == 3) {
      for (let i = 0; i < 3; i++) {
        ...
        ledContainer.children[i].style.webkitBoxShadow =
